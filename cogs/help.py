...
class HelpCog(commands.Cog):
    def __init__(self, _bot):
        self.bot = _bot
        help_attributes = {
            'help': 'Displays help for the bot\'s commands',
            'hidden': True,
...
class MyHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        embed = embed_create(self.context.author,
                             title='Help:',
                             description=f'`-help [command]` for command info\n'
    ...
    async def send_group_help(self, group):
        ...
        embed.add_field(
            name='Help:', value=f'{group.short_doc}\n', inline=False)
        embed.add_field(name='Subcommand Help:', value='᲼', inline=False)
        for command in group.commands:
            embed.add_field(name=self.get_command_signature(
                command), value=command.help, inline=False)
        channel = self.get_destination()
        await channel.send(embed=embed)

    async def send_error_message(self, error):
        embed = embed_create(
            self.context.author, title='Help command error!', description=error, color=0xeb4034)
        channel = self.get_destination()
        await channel.send(embed=embed)
