...
from classes import *
from classes import embed_create, CustomMenu
...
class ReminderList(menus.ListPageSource):
    async def format_page(self, menu, entries):
        ...
        for reminder in entries:
            channel = reminder.destination if isinstance(
                reminder.destination, TextChannel) else None
...
class Reminder:
    ...
    def __post_init__(self):
        self.duration = 1 if self.end_time - \
            time.time() <= 0 else self.end_time - time.time()
    ...
    async def send_reminder(self):
        ...
        try:
            await self.destination.send(
                f"**Hey {self.user.mention},**" if isinstance(
                    self.destination, TextChannel) else None,
                embed=embed)
    ...
    async def load_reminders():
        async with bot.db.cursor() as cursor:
            for row in await cursor.execute('SELECT * FROM reminders'):
                ...
                reminder: str = row[2]
                end_time: int = row[3]
                destination: Union[User, TextChannel] = bot.get_channel(
                    row[4]) or user
...
class ReminderCog(commands.Cog, name="Reminder Commands!"):
        ...
        *remind 1hr 30m do stuff*"""
        ...
        if not durations:
            raise commands.BadArgument(
                'The duration wasn\'t specified or it was invalid!')

        if len(durations) != len(durations_set):
            raise commands.BadArgument(
                'There were duplicate units in the duration!')
        ...
        rem = Reminder(ctx.message.id, ctx.author,
                       reminder, destination, end_time)
    ...
    async def reminders(self, ctx):
        ...
        menu = CustomMenu(source=ReminderList(
            filtered_reminders, per_page=5), clear_reactions_after=True)
    ...
    async def delete(self, ctx, reminder_id: int):
        ...
        if reminder is None:
            raise commands.BadArgument(
                'A reminder with that ID wasn\'t found!')
