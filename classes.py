...
__all__ = ['CustomBot', 'TimeConverter',
           'MentionedTextChannel', 'seconds_to_str']
...
def embed_create(user, **kwargs):
    ...
    embed = discord.Embed(description=description,
                          title=title, color=color, url=url)
    embed.set_footer(
...
class CustomBot(commands.Bot):
    ...
    async def on_message(self, message):
        # if message is by bot, ignore
        if message.author.bot:
            return

        # handles @ the bot instead of prefix commands
...
class TimeConverter(commands.Converter):
    ...
    async def convert(self, _, argument: str):
        ...
        try:
            ...
            raise commands.BadArgument(amount, unit)
...
class MentionedTextChannel(commands.Converter):
    async def convert(self, ctx, argument) -> discord.TextChannel:
        match = ID_REGEX.match(argument) or re.match(
